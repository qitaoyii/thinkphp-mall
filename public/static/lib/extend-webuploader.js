jQuery(function() {
    ...
    function addFile(file) {
        ...
        if (file.size < 5000000) {
            ...
            window.sessionStorage.setItem('imgnum', index + 1);
            var imgLeft = index * (thumbnailWidth + 10);
            var imgTop = 0;
            var wrapHeight = thumbnailHeight;
            var wrapWidth = $queue.width() + 20 + 10;
            if (imgLeft >= wrapWidth) {
                imgTop = parseInt(imgLeft / wrapWidth) * (thumbnailHeight);
            ...
            $li.appendTo($queue);
            // console.log(index + 1);
            // console.log(window.sessionStorage.getItem('video') == false)

            if (window.sessionStorage.getItem('video') == 1) {
                console.log(1)
                if ((index + 1) <= 4) {
                    $('.product-file-img-box').width((index + 1) * 110) + "px";
                    $('.product-add-img').show();
                }

                if ((index + 1) == 4) {
                    $('.product-add-img').hide();
                }
    ...
    function removeFile(file) {
        ...
        var index = $queue.find('li').length;
        window.sessionStorage.setItem('imgnum', index);
        if (window.sessionStorage.getItem('video') + index < 5) {
            $('.product-add-img').show();
        } else {
            $('.product-add-img').show();
        }
        $('.product-file-img-box').width((index * 110) + "px");
        if (window.sessionStorage.getItem('video') == false) {
            if (index < 5) {
                $('.product-add-img').show();
            }
        } else {
